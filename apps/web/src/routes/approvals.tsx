      ...
      <main className="mx-auto flex w-full max-w-screen-lg flex-col gap-6 px-8 py-8">
        ...
        <div className="grid gap-3">
          {(replicatedApprovals ?? approvalsQuery.data)?.map((approval) => (
            <article key={approval.id} className="grid gap-4 rounded-lg border border-border/70 bg-card/30 p-4">
              <div className="flex items-start justify-between gap-4"><div><Link to="/$organizationSlug/teams/$teamKey/issues/$issueId" params={{ organizationSlug, teamKey: approval.team_key, issueId: approval.issue_id }} onClick={() => selectProject(approval.project_id)} className="font-medium hover:underline">{approval.issue_title}</Link><p className="mt-1 text-xs text-muted-foreground">{approval.project_name} · requested by {approval.requested_by.slice(0, 8)} · target version {approval.target_version}</p></div><Badge variant="outline">pending</Badge></div>
              <pre className="overflow-auto rounded-md bg-muted/40 p-3 text-xs">{JSON.stringify(approval.proposed_operation, null, 2)}</pre>
              <div className="flex items-center justify-end gap-2"><Button variant="ghost" size="sm" onClick={() => decisionMutation.mutate({ projectId: approval.project_id, approvalId: approval.id, approve: false })} disabled={decisionMutation.isPending}><X /> Reject</Button><Button size="sm" onClick={() => decisionMutation.mutate({ projectId: approval.project_id, approvalId: approval.id, approve: true })} disabled={decisionMutation.isPending}><Check /> Approve</Button></div>
            </article>
          ))}
        </div>
      </main>
