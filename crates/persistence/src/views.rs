...
impl super::Database {
    pub async fn list_saved_views(&self, account_id: Uuid) -> Result<Vec<SavedViewRecord>, Error> {
        Ok(sqlx::query_as::<_, SavedViewRecord>(
            "SELECT id, account_id, name, filters, created_at, updated_at
    ...
    pub async fn save_view(
        ...
        Ok(sqlx::query_as::<_, SavedViewRecord>(
            "INSERT INTO human_saved_views (id, account_id, name, filters)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (account_id, name) DO UPDATE
             SET filters = EXCLUDED.filters, updated_at = now()
             RETURNING id, account_id, name, filters, created_at, updated_at",
        )
        .bind(Uuid::now_v7())
        .bind(account_id)
        .bind(name)
